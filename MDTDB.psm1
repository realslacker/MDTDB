...
function Copy-MDTComputer {

    [CmdletBinding()]
    PARAM
    (
        [Parameter(ValueFromPipelineByPropertyName=$true)] $copyId,
        ...
        [Parameter(ValueFromPipelineByPropertyName=$true)] $settings=@{}
    )

    Process
    {
        #get the original MM
        $original = Get-MDTComputer -id $copyId

        #disallowed properties
        $badprops = 'AssetTag','UUID','SerialNumber','MacAddress','ID','Type','RowError','RowState','Table','ItemArray','HasErrors'
        
        #select the settings
        $original.PSObject.Properties | ForEach-Object {
            If ( -not( $badprops.Contains( $_.Name ) ) -and -not( [string]::IsNullOrEmpty( $_.Value ) ) ) {

                if ( $settings.Keys -contains $_.Name ) {

                    Write-Host "Skipping old $($_.Name)..."
        ...
        New-MDTComputer -assetTag $assetTag -macAddress $macAddress -serialNumber $serialNumber -uuid $uuid -description $description -settings $settings
    }
}
...
function New-MDTComputer {
    ...
    Process
    {
        # fix the mac address to make MDT happy
...
function Copy-MDTMakeModel {

    [CmdletBinding()]
    PARAM
    (
        [Parameter(ValueFromPipelineByPropertyName=$true)] $copyId,
        [Parameter(ValueFromPipelineByPropertyName=$true)] $newMake,
        [Parameter(ValueFromPipelineByPropertyName=$true)] $newModel
    )

    Process
    {
        #get the original MM
        $original = Get-MDTMakeModel -id $copyId

        #disallowed properties
        $badprops = 'Make','Model','Type','ID','RowError','RowState','Table','ItemArray','HasErrors'
        ...
        New-MDTMakeModel -make $newMake -model $newModel -settings $settings
    }
}
